...
void Renderer::Render(Camera* camera)
{
	...
	targets[4] = renderTargetRTVs[RenderTargetType::SCENE_VELOCITIES].Get();
	...
	// Collect all per-frame data and copy to GPU
	{
		...
		vsPerFrameData.PrevFrameViewMatrix = prevFrameView;
		vsPerFrameData.PrevFrameProjectionMatrix = prevFrameProj;
	...
	for (auto ge : toDraw)
	{
		...
		if (currentVS != 0)
		{
			...
			currentVS->SetMatrix4x4("prevWorld", trans->GetPrevFrameWorldMatrix());
	...
	targets[0] = renderTargetRTVs[RenderTargetType::SCENE_COLORS_NO_AMBIENT].Get();
	targets[1] = renderTargetRTVs[RenderTargetType::SCENE_VELOCITIES].Get();
	targets[2] = 0;
	targets[3] = 0;
	targets[4] = 0;
	context->OMSetRenderTargets(numTargets, targets, depthBufferDSV.Get());
	...
	// Final combine
	{
		// Final combine of SSAO + scene
		targets[0] = renderTargetRTVs[RenderTargetType::FINAL_COMBINE].Get();
		context->OMSetRenderTargets(1, targets, 0);
	...
	// Save last frame's camera matrices
	prevFrameView = camera->GetView();
	prevFrameProj = camera->GetProjection();
}
