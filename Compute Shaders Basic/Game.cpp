...
void Game::LoadAssetsAndCreateEntities()
{
	...
	// Create the noise texture for our compute shader to use
	...
	// Create a simple material that uses the compute shader texture
	Material* noiseMaterial = new Material(vs, assets.GetPixelShader("SimpleTextureMRTPS.cso"), XMFLOAT4(1, 1, 1, 1), 0.0f, XMFLOAT2(1, 1));
	noiseMaterial->AddPSTextureSRV("Pixels", computeTextureSRV);
	noiseMaterial->AddPSSampler("BasicSampler", samplerOptions);
	materials.push_back(noiseMaterial);
	...
	// Create a sphere to show the noise texture
	GameEntity* noiseSphere = new GameEntity(assets.GetMesh("Models\\sphere.obj"), noiseMaterial);
	entities.push_back(noiseSphere);
	noiseSphere->GetTransform()->MoveAbsolute(0, 0.5f, 0);
...
void Game::CreateUI(float dt)
{
	...
	if (ImGui::CollapsingHeader("Compute Shader Options"))
	{
		ImVec2 size = ImGui::GetItemRectSize();

		if (ImGui::Button(computeShaderActive ? "Compute Shader Running" : "Compute Shader Disabled"))
			computeShaderActive = !computeShaderActive;
		...
		ImageWithHover(computeTextureSRV.Get(), ImVec2(size.x, size.x));
	}
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	if (computeShaderActive)
	{
		...
		// Grab the compute shader
		SimpleComputeShader* noiseCS = Assets::GetInstance().GetComputeShader("NoiseTextureCS.cso");
