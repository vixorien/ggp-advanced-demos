...
void Game::BuildUI()
{
	...
	ImGui::Begin("Inspector");
	{
		...
		if (ImGui::TreeNode("Renderer"))
		{
			...
			// Post-processing
			ImGui::Checkbox("Post Process Active", &renderer->postProcessActive);
			if (renderer->postProcessActive)
			{
				// Show PP options
				ImGui::DragFloat("Chromatic Aber. Offset", &renderer->chromaticAberrationOffset, 0.001f, -0.1f, 0.1f);
			
				// Draw render targets
				ImVec2 size = ImGui::GetItemRectSize();
				float rtHeight = size.x * ((float)windowHeight / windowWidth);

				ImageWithHover(renderer->postProcessSRV.Get(), ImVec2(size.x, rtHeight));
			}
