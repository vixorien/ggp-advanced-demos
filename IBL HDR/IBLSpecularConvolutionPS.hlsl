...
float3 ConvolveTextureCube(float roughness, float3 R)
{
	...
	for (uint i = 0; i < MAX_SAMPLES; i++)
	{
		...
		if (nDotL > 0)
		{
			...
			float dist = SpecDistribution(N, H, roughness);
			float pdf = dist; // (dist * nDotH / (4.0f * hDotV)) + 0.0001f;

			float s = 1.0f / (MAX_SAMPLES * pdf + 0.0001f);
			float p = 1.0f / (fullResolution * fullResolution);

			float mipToSample = max(0.5f * log2(s / p), 0);
			mipToSample *= totalMips;

			// Sample
			float3 thisColor = EnvironmentMap.SampleLevel(BasicSampler, L, mipToSample).rgb;
			thisColor = needsGammaCorrection == 1 ? pow(thisColor, 2.2f) : thisColor; 
