...
bool Sky::GetNeedsGammaCorrection() { return needsGammaCorrection; }
...
void Sky::IBLCreateIrradianceMap()
{
	...
	for (int face = 0; face < 6; face++)
	{
		...
		irradiancePS->SetInt("needsGammaCorrection", (int)needsGammaCorrection);
...
void Sky::IBLCreateConvolvedSpecularMap()
{
	...
	int fullResoluation = pow(2, totalSpecIBLMipLevels + specIBLMipLevelsToSkip - 1);
	// == Render convolution =======================================
	...
	for (int mip = 0; mip < totalSpecIBLMipLevels; mip++)
	{
		...
		for (int face = 0; face < 6; face++)
		{
			...
			specConvPS->SetInt("totalMips", totalSpecIBLMipLevels + specIBLMipLevelsToSkip);
			specConvPS->SetInt("fullResolution", fullResoluation);
			specConvPS->SetInt("needsGammaCorrection", needsGammaCorrection);
