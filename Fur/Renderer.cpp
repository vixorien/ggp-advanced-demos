...
void Renderer::Render(Camera* camera, float time)
{
	...
	for (auto ge : toDraw)
	{
		...
		if (currentMesh != 0)
		{
			// Is this a fur material?
			int instanceCount = currentMaterial->IsFur() ? furShellCount : 1;

			// Hardcoding some extra GPU work here
			if (currentMaterial->IsFur())
			{
				currentMaterial->GetVS()->SetFloat("furShellOffset", furShellOffset);
				currentMaterial->GetVS()->CopyBufferData("perObject");

				currentMaterial->GetPS()->SetInt("furShellCount", furShellCount);
				currentMaterial->GetPS()->SetFloat("furMapCutoff", furMapCutoff);
				currentMaterial->GetPS()->SetFloat("furDepthDarken", furDepthDarken);
				currentMaterial->GetPS()->SetFloat2("furShellUVTwist", furShellUVTwist);
				currentMaterial->GetPS()->SetFloat("time", time);
				currentMaterial->GetPS()->CopyBufferData("perObject");
			}

			context->DrawIndexedInstanced(currentMesh->GetIndexCount(), instanceCount, 0, 0, 0);
		}
	}
