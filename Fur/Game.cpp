...
void Game::LoadAssetsAndCreateEntities()
{
	...
	assets.CreateSolidColorTexture("lightGrey", 2, 2, XMFLOAT4(0.75f, 0.75f, 0.75f, 1));
	...
	// Fur testing
	Material* furMat = new Material(
		assets.GetVertexShader("FurVS.cso"),
		assets.GetPixelShader("FurPS.cso"),
		XMFLOAT4(1, 1, 1, 1),
		0,
		XMFLOAT2(1, 1),
		true);
	furMat->AddPSTextureSRV("AlbedoTexture", assets.GetTexture("Textures\\tigerstripe.png"));
	furMat->AddPSTextureSRV("NormalTexture", assets.GetTexture("flatNormalMap"));
	furMat->AddPSTextureSRV("RoughnessTexture", assets.GetTexture("lightGrey"));
	furMat->AddPSTextureSRV("MetalTexture", assets.GetTexture("black"));
	furMat->AddPSTextureSRV("FurTexture", assets.GetTexture("Textures\\furMap.png"));
	furMat->AddPSSampler("BasicSampler", samplerOptions);
	furMat->AddPSSampler("ClampSampler", clampSampler);
	materials.push_back(furMat);

	GameEntity* fur = new GameEntity(sphereMesh, furMat);
	fur->GetTransform()->SetScale(3, 3, 3);
	entities.push_back(fur);
}
...
void Game::CreateUI(float dt)
{
	...
	// SSAO Options
	if (ImGui::CollapsingHeader("Fur"))
	{
		ImGui::SliderInt("Fur Shell Count", &renderer->furShellCount, 1, 50);
		ImGui::SliderFloat("Fur Shell Height Offset", &renderer->furShellOffset, 0.0f, 0.005f, "%.5f");
		ImGui::SliderFloat("Fur Darkening", &renderer->furDepthDarken, 0.0f, 1.0f);
		ImGui::SliderFloat("Fur Map Alpha Cutoff", &renderer->furMapCutoff, 0.0f, 1.0f);
		ImGui::SliderFloat2("Fur Map UV Twist", &renderer->furShellUVTwist.x, -0.001f, 0.001f, "%.5f");
	}
