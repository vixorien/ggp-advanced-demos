...
void Game::Init()
{
	...
	renderer = new Renderer(
		entities,
		lights,
		3, // Only directional lights to start
		sky,
		width,
		height,
		device,
...
void Game::CreateUI(float dt)
{
	...
	// Shadow Options
	if (ImGui::CollapsingHeader("Shadow Mapping Options"))
	{
		ImVec2 size = ImGui::GetItemRectSize();

		ImGui::Text("NOTE: This demo makes several assumptions:\n 1. There is exactly 1 shadow-casting light\n 2. The shadow-casting light is the first light in the list\n 3. The shadow-casting light is a directional light\n\n");

		unsigned int shadowMapRes = renderer->GetShadowMapResolution();
		ImGui::Text((std::string("Shadow map resolution: ") + std::to_string(shadowMapRes)).c_str());

		ImGui::Text("Change size: "); ImGui::SameLine();
		if (ImGui::Button("128")) { renderer->SetShadowMapResolution(128); } ImGui::SameLine();
		if (ImGui::Button("256")) { renderer->SetShadowMapResolution(256); } ImGui::SameLine();
		if (ImGui::Button("512")) { renderer->SetShadowMapResolution(512); } ImGui::SameLine();
		if (ImGui::Button("1024")) { renderer->SetShadowMapResolution(1024); } ImGui::SameLine();
		if (ImGui::Button("2048")) { renderer->SetShadowMapResolution(2048); } ImGui::SameLine();
		if (ImGui::Button("4096")) { renderer->SetShadowMapResolution(4096); }

		float projSize = renderer->GetShadowProjectionSize();
		if (ImGui::SliderFloat("Projection size", &projSize, 1.0f, 50.0f))
			renderer->SetShadowProjectionSize(projSize);

		ImageWithHover(renderer->GetShadowMapSRV().Get(), ImVec2(size.x, size.x));
	}
	...
	if (ImGui::CollapsingHeader("All Render Targets"))
	{
		...
		ImageWithHover(renderer->GetShadowMapSRV().Get(), ImVec2(rtHeight, rtHeight));
