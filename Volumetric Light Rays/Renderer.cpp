...
void Renderer::Render(Camera* camera)
{
	...
	// Final combine of SSAO and scene
	{
		// Set up SSAO combine target (assuming all other targets are null here)
		targets[0] = renderTargetRTVs[RenderTargetType::SSAO_COMBINE].Get();
		context->OMSetRenderTargets(1, targets, 0);
	...
	// Raymarch for volumetric light as the last step
	{
		...
		// Draw
		SimplePixelShader* ps = assets.GetPixelShader("LightRayPS.cso");
		ps->SetShader();
		ps->SetShaderResourceView("SkyAndOccluders", renderTargetSRVs[RenderTargetType::SCENE_SKY_AND_OCCLUDERS]);
		ps->SetShaderResourceView("FinalScene", renderTargetSRVs[RenderTargetType::SSAO_COMBINE]);
	...
	Microsoft::WRL::ComPtr<ID3D11DepthStencilView> depthBufferDSV)
{
	...
	CreateRenderTarget(windowWidth, windowHeight, renderTargetRTVs[RenderTargetType::SCENE_SKY_AND_OCCLUDERS], renderTargetSRVs[RenderTargetType::SCENE_SKY_AND_OCCLUDERS]);
	...
	CreateRenderTarget(windowWidth, windowHeight, renderTargetRTVs[RenderTargetType::SSAO_COMBINE], renderTargetSRVs[RenderTargetType::SSAO_COMBINE]);
}
