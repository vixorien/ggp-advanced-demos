...
void Game::Update(float deltaTime, float totalTime)
{
	...
	// Tree (2nd to last entity)
	entities[entities.size() - 2]->GetTransform()->Rotate(0, deltaTime * 0.1f, 0);
...
void Game::CreateUI(float dt)
{
	...
	// Light ray Options
	if (ImGui::CollapsingHeader("Volumetric Light Rays"))
	{
		ImGui::Indent(10.0f);
		...
		ImGui::SliderInt("Num Samples", &renderer->lightRaySamples, 2, 256);
		ImGui::SliderFloat("Volume Density", &renderer->lightRayDensity, 0.01f, 1.0f);
		ImGui::SliderFloat("Sample Weight", &renderer->lightRaySampleWeight, 0.0f, 1.0f);
		ImGui::SliderFloat("Light Decay", &renderer->lightRayDecay, 0.0f, 1.0f);
		ImGui::SliderFloat("Overall Exposure", &renderer->lightRayExposure, 0.0f, 2.0f);
