...
Texture2D SunAndOccluders		: register(t1);
...
float4 main(VertexToPixel input) : SV_TARGET
{
	...
	float3 rayColor = SunAndOccluders.Sample(BasicSampler, input.uv).rgb;
	...
	for (int i = 0; i < numSamples; i++)
	{
		...
		rayPos -= rayDir;
		float3 stepColor = SunAndOccluders.Sample(BasicSampler, rayPos).rgb;
