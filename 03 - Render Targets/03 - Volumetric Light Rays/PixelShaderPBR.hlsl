...
PS_Output main(VertexToPixel input)
{
	...
	// NOTE: Ensure your energy conservation function takes FLOAT3's for diffuse & specular!
	...
	totalColor += fullIndirect * iblIntensity * indirectLightingEnabled;

	// Multiple render target output
	PS_Output output;
	output.sceneColor		= float4(totalColor, 1); // No gamma correction yet!
	output.sunAndOccluders = float4(0, 0, 0, 1); // Black since this object occludes the sky
	return output;
}