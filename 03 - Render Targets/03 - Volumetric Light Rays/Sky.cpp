	...
	Microsoft::WRL::ComPtr<ID3D11DeviceContext> context)
{
	...
	nlohmann::json t = jsonSky["textures"];
...
void Sky::IBLCreateIrradianceMap()
{
	...
	// == Set up shaders ============================================
	Assets& assets = Assets::GetInstance();
	std::shared_ptr<SimpleVertexShader> fullscreenVS = assets.GetVertexShader(L"FullscreenVS");
	std::shared_ptr<SimplePixelShader> irradiancePS = assets.GetPixelShader(L"IBLIrradianceMapPS");
...
void Sky::IBLCreateConvolvedSpecularMap()
{
	...
	Assets& assets = Assets::GetInstance();
	std::shared_ptr<SimpleVertexShader> fullscreenVS = assets.GetVertexShader(L"FullscreenVS");
	std::shared_ptr<SimplePixelShader> specConvPS = assets.GetPixelShader(L"IBLSpecularConvolutionPS");
...
void Sky::IBLCreateBRDFLookUpTexture()
{
	...
	Assets& assets = Assets::GetInstance();
	std::shared_ptr<SimpleVertexShader> fullscreenVS = assets.GetVertexShader(L"FullscreenVS");
	std::shared_ptr<SimplePixelShader> envBrdfPS = assets.GetPixelShader(L"IBLBrdfLookUpTablePS");
